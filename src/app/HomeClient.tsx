...
async function saveBouquetToAppdrop({
  ...
  await window.appdrop.saveOutput({
    ...
    source_url: url,
    // Appdrop chat publishing is a separate opt-in from Link Bouquet's own
    // public gallery. Keep the output private until the user taps Send.
    visibility: "private",
    data: {
      bg_color: bgColor,
...
function Home() {
  ...
  const handleSave = async () => {
    ...
    await saveBouquetToAppdrop({
      ...
      // The standalone Link Bouquet save should still finish if Appdrop is
      // unavailable, while the parent can surface its own save error.
